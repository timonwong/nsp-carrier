...
const baseSnapshot = {
  ...
  hasConflict: false,
  lastError: "",
  activeProfile: "goldleaf",
  validationErrors: [],
  profiles: [
    {
      id: "awoo",
      displayName: "Backend Awoo",
      protocolFamily: "Awoo USB",
      transport: "usb",
      supportedExtensions: [".nsp"],
      wireNamespace: "flat-basename",
      filesystemAccess: "none",
      compatibleVersions: [],
      verifiedVersions: ["1.6.2"],
      knownIncompatibleVersions: [],
      adapterAvailable: true,
    },
    {
      id: "goldleaf",
      displayName: "Backend Goldleaf",
      protocolFamily: "Goldleaf 0.10+",
      transport: "usb",
      supportedExtensions: [".foo"],
      wireNamespace: "VIRT:/",
      filesystemAccess: "read-only",
      compatibleVersions: ["0.10+"],
      verifiedVersions: [],
      knownIncompatibleVersions: [],
      adapterAvailable: true,
    },
    {
      id: "sphaira",
      displayName: "Backend Sphaira",
      protocolFamily: "Sphaira SPH0",
      transport: "usb",
      supportedExtensions: [".nsp", ".msp"],
      wireNamespace: "flat-basename",
      filesystemAccess: "none",
      compatibleVersions: ["1.0+"],
      verifiedVersions: [],
      knownIncompatibleVersions: ["0.13.3 and earlier"],
      adapterAvailable: true,
    },
    {
      id: "dbi",
      displayName: "Backend DBI",
      protocolFamily: "DBI0",
      transport: "usb",
      supportedExtensions: [".nsp", ".nsz"],
      wireNamespace: "flat-basename",
      filesystemAccess: "none",
      compatibleVersions: ["DBI0"],
      verifiedVersions: [],
      knownIncompatibleVersions: [],
      adapterAvailable: true,
    },
...
describe("installer profile UI", () => {
  ...
  it("separates session state from the current serving action", async () => {
    ...
    expect(
      screen.getByText("Fully Served ≠ installed. Compatibility ≠ verified version."),
    ).toBeTruthy();
  });
  ...
  it("shows item-specific validation without deselecting the file", async () => {
    ...
    expect(
      (screen.getByRole("checkbox", { name: "Select compressed.nsz" }) as HTMLInputElement).checked,
    ).toBe(true);
  });
});
